...
function changeActiveTab(name, index) {
  const oldActiveBtn = document.querySelector(`.tabs-btns[data-tabs-name='${name}'] > .tabs-btns__item--active`);
  const oldActiveTab = document.querySelector(`.tabs-list[data-tabs-name='${name}'] > .tab-item--active`);

  const newActiveBtn = document.querySelectorAll(`.tabs-btns[data-tabs-name='${name}'] > .tabs-btns__item`)[index];
  const newActiveTab = document.querySelectorAll(`.tabs-list[data-tabs-name='${name}'] > .tab-item`)[index];
...
function toggleAccordion() {
  ...
  filters.forEach((filter) => {
    ...
    if (filter.classList.contains("tab-item--open") && window.innerWidth <= 768) {
